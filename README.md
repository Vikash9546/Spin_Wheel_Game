...
### SQL Migrations
A custom migration file is available at [backend/prisma/migrations/20260521170000_add_user_credentials/migration.sql](file:///Users/vikashkumar/Desktop/RoxStar/backend/prisma/migrations/20260521170000_add_user_credentials/migration.sql). It is used to patch schemas for production deployment and handle legacy players:

```sql
-- AlterTable: Add columns to users table safely
ALTER TABLE "users" ADD COLUMN IF NOT EXISTS "email" TEXT;
ALTER TABLE "users" ADD COLUMN IF NOT EXISTS "password_hash" TEXT;

-- Migration: Generate email addresses dynamically for legacy players
UPDATE "users"
SET "email" = lower(regexp_replace("id", '[^a-zA-Z0-9]+', '_', 'g')) || '@legacy.local'
WHERE "email" IS NULL;

-- Migration: Set fallback passwords for legacy players
UPDATE "users"
